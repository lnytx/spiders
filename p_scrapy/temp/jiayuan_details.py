...
class jiayuan_data(RedisSpider):
    name = "jiayuan_details"
    redis_key = 'jiayuan_main:start_urls'
    #allowed_domains = ["jiayuan.com"]
    # 启动爬虫的命令
    redis_key = "sinaspider:strat_urls"
    start_urls = []
    for p in xrange(1,2000000):
            search_url = "http://search.jiayuan.com/v2/index.php?key=&sex=f&stc=&sn=default&sv=1&p=%s&pt=%s&ft=off&f=select&mt=d" %(p,2000000)
...
#     ]
    ...
    def start_requests(self):#
        ...
        for url in self.start_urls:
            ...
            yield scrapy.Request(url=url,meta={'result':result},callback=self.get_main_info)
